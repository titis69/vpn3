...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/Vlukss/izinvps/main/ip | grep $MYIP | awk '{print $2}')
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "prikitiuwkdm@gmail.com" &> /dev/null
    git config --global user.name "Vlukss" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/Vlukss/backup
    git push -f https://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@github.com/Vlukss/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/Vlukss/backup/main/$NameUser/$NameUser.zip"
sleep 1
